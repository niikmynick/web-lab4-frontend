...
export const fetchData = createAsyncThunk('data/fetchData', async () => {
    ...
    const response = await fetch(`http://localhost:8080/server-1.0-SNAPSHOT/api/data/get?owner_id=${ownerId}`, {
        method: 'POST',
        headers: {
            Authorization: token,
        },
...
export const sendData = createAsyncThunk('data/sendData', async ({ x, y, r }) => {
    ...
    const response = await fetch(
        `http://localhost:8080/server-1.0-SNAPSHOT/api/data/add?x=${x}&y=${y}&r=${r}&owner_id=${ownerId}`,
        {
            method: 'POST',
            headers: {
                Authorization: token,
...
export const clearData = createAsyncThunk('data/clearData', async () => {
    ...
    const response = await fetch(`http://localhost:8080/server-1.0-SNAPSHOT/api/data/clear?owner_id=${ownerId}`, {
        method: 'POST',
        headers: {
            Authorization: token,
        },
