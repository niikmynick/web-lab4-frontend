...
export const login = createAsyncThunk('auth/login', async ({ username, password }, { dispatch }) => {
    try {
        const response = await fetch('http://localhost:8080/server-1.0-SNAPSHOT/api/auth/login', {
            method: 'POST',
            headers: {
...
export const register = createAsyncThunk('auth/register', async ({ username, password }, { dispatch }) => {
    try {
        const response = await fetch('http://localhost:8080/server-1.0-SNAPSHOT/api/auth/register?username=', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
